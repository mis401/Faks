...
public class ZahtevServiceImplementation implements ZahtevService {
	...
	/*
	 * public ZahtevServiceImplementation() { EntityManagerFactory emf =
	...
	public double ukupnaCena(int id) {
		...
		Zahtev zahtev = em.find(Zahtev.class, id);
		if (zahtev == null)
			throw new Exception("Nema zahteva");
		double cena = 0;
		List<Instrument> instrumenti = nadjiInstrumente(zahtev.getId());
		cena += em.find(Sala.class, zahtev.getSalaId()).getCena();
		for (Instrument i : instrumenti) {
			cena += i.getCena();
		}
		if (zahtev.getUsluge()) {
			cena+=zahtev.getBrojPesama()*100;
		}
		//em.getTransaction().begin();
		zahtev.setCena(cena);
		//em.getTransaction().commit();
